...
#   bash bench/run.sh                 # captures a 60s reference of your screen
#   bash bench/run.sh 90              # captures a 90s reference
#   bash bench/run.sh path/to/ref.mov # uses an existing clip (should be ProRes/lossless)
#
# Capturing needs Screen Recording permission for Terminal. While it records,
# do a realistic demo: scroll, type, switch apps, move windows.
#
...
ARG="${1:-60}"
if [ -f "$ARG" ]; then
  echo "== Normalising supplied clip to ${W}x${H}@${FPS} ProRes HQ =="
  ffmpeg -hide_banner -loglevel error -y -i "$ARG" -an \
  ...
  swiftc -O "$ROOT/bench/capture.swift" -o "$OUT/capture-bin" || exit 1
  echo "== Capturing ${DUR}s of your main display via ScreenCaptureKit. Start demoing at GO =="
  for i in 3 2 1; do echo "  ${i}..."; sleep 1; done
  echo "  GO"
  "$OUT/capture-bin" "$RAW" "$DUR" || exit 1
  echo "== Normalising capture to constant ${FPS}fps =="
  ffmpeg -hide_banner -loglevel error -y -i "$RAW" -vf "fps=${FPS}" \
    -c:v prores_ks -profile:v 3 -pix_fmt yuv422p10le "$REF" || exit 1
  rm -f "$RAW"
fi
...
with open(sys.argv[3], "w") as f:
    f.write(f"# Mercury compression benchmark\n\nReference: {dur:.0f}s, 1920x1080@60, ProRes HQ. Scores are against the reference (higher is better).\n")
