...
### Changed
- **Resolution: 1080p / 4K** — replaces the 1x / 1.5x / 2x scale picker. 1080p is the default (Loom-sized files, ~12 MB/min on Balanced); 4K captures Retina and roughly doubles the size.
- **4K output for sharp sources** — when the capture is finer than 1080p (Resolution 1.5x/2x, or the iPhone's native screen), the canvas is 3840 or 2160 wide instead of downscaling to 1080p, so text keeps its pixels. Balanced quality nudged to 0.70.
